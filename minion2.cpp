...
void minion2::enemyStateRender()
{
	...
	{
	case E_IDLE:
		if (_left) FINDIMG("enemy2_idle")->frameRender(getMemDC(), _rc.left - 40, _rc.top - 10, _index, 1);
		else FINDIMG("enemy2_idle")->frameRender(getMemDC(), _rc.left - 80, _rc.top - 10, _index, 0);		break;
	case E_WALK:
		if (_left) FINDIMG("enemy2_move")->frameRender(getMemDC(), _rc.left - 75, _rc.top - 28, _index, 1);
		else FINDIMG("enemy2_move")->frameRender(getMemDC(), _rc.left - 75, _rc.top - 28, _index, 0);		break;
	case E_ATK:
		if (_left) FINDIMG("enemy2_atk")->frameRender(getMemDC(), _rc.left - 130, _rc.top - 160, _index, 1);
		else FINDIMG("enemy2_atk")->frameRender(getMemDC(), _rc.left - 50, _rc.top - 160, _index, 0);		break;
	case E_DEAD:
		if (_left) FINDIMG("enemy2_dead")->frameRender(getMemDC(), _rc.left - 60, _rc.top - 30, _index, 1);
		else FINDIMG("enemy2_dead")->frameRender(getMemDC(), _rc.left - 60, _rc.top - 30, _index, 0);		break;
	case E_HIT:
		if (_left) FINDIMG("enemy2_hit")->frameRender(getMemDC(), _rc.left - 65, _rc.top - 43, _index, 1);
		else FINDIMG("enemy2_hit")->frameRender(getMemDC(), _rc.left - 65, _rc.top - 43, _index, 0);		break;
	//case E_GRAB:
